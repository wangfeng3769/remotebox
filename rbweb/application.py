...
class application:
    ...
    def runbasic(self, *middleware):
        import rbweb.httpserver as websrv
        return websrv.runbasic(self.wsgifunc(*middleware),self.addr)
        # return wsgi.runwsgi(self.wsgifunc(*middleware),self.addr)
    ...
    def get_parent_app(self):
        ...
        if self in web.ctx.app_stack:
            index = cctx.app_stack.index(self)
            if index > 0:
                return cctx.app_stack[index-1]
...
class auto_application(application):
    def __init__(self):
        ...
        class metapage(type):
            def __init__(self,klass, name, bases, attrs):
